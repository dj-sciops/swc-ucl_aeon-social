...
class ActivityWeightScale(dj.Manual):
        definition = """ # activity_weight_scale operation for time, location, experiment (v-0.4.7)
...
class Camera(dj.Manual):
        definition = """ # camera operation for time, location, experiment (v-0.4.7)
        -> acquisition.Experiment
        -> DeviceName
...
class Environment(dj.Manual):
        definition = """ # environment operation for time, location, experiment (v-0.4.7)
        -> acquisition.Experiment
        -> DeviceName
...
class Feeder(dj.Manual):
        definition = """ # feeder operation for time, location, experiment (v-0.4.7)
        -> acquisition.Experiment
        -> DeviceName
...
@schema 
class EnvironmentMessageLog(dj.Imported):
        definition = """ # Raw per-chunk MessageLog from Environment (v-0.4.7)
    -> Environment
    -> acquisition.Chunk
    ---
    sample_count: int32      # number of data points
    timestamps: json         # time range, sampling rate, count
    priority: json             # summary stats
    type: json             # summary stats
    message: json             # summary stats
    ...
    """
        ...
        def make(self, key):
            ...
            stream_reader = get_stream_reader_for_epoch(
                key["experiment_name"], device_name, "MessageLog", epoch_start
            )
...
class ActivityWeightScaleWeightFiltered(dj.Imported):
        definition = """ # Raw per-chunk WeightFiltered from ActivityWeightScale (v-0.4.7)
    -> ActivityWeightScale
    -> acquisition.Chunk
    ---
    sample_count: int32      # number of data points
...
class FeederDeliverPellet(dj.Imported):
        definition = """ # Raw per-chunk DeliverPellet from Feeder (v-0.4.7)
    -> Feeder
    -> acquisition.Chunk
    ---
    sample_count: int32      # number of data points
...
class FeederBeamBreak(dj.Imported):
        definition = """ # Raw per-chunk BeamBreak from Feeder (v-0.4.7)
    -> Feeder
    -> acquisition.Chunk
    ---
    sample_count: int32      # number of data points
...
class FeederEncoder(dj.Imported):
        definition = """ # Raw per-chunk Encoder from Feeder (v-0.4.7)
    -> Feeder
    -> acquisition.Chunk
    ---
    sample_count: int32      # number of data points
...
@schema 
class EnvironmentEnvironmentState(dj.Imported):
        definition = """ # Raw per-chunk EnvironmentState from Environment (v-0.4.7)
    -> Environment
    -> acquisition.Chunk
    ---
    sample_count: int32      # number of data points
    timestamps: json         # time range, sampling rate, count
    state: json             # summary stats
    stream_df: <aeon_stream>   # full DataFrame via codec
    """
        ...
        def make(self, key):
            ...
            stream_reader = get_stream_reader_for_epoch(
                key["experiment_name"], device_name, "EnvironmentState", epoch_start
            )
...
class CameraVideo(dj.Imported):
        definition = """ # Raw per-chunk Video from Camera (v-0.4.7)
    -> Camera
    -> acquisition.Chunk
    ---
    sample_count: int32      # number of data points
...
class EnvironmentLightEvents(dj.Imported):
        definition = """ # Raw per-chunk LightEvents from Environment (v-0.4.7)
    -> Environment
    -> acquisition.Chunk
    ---
    sample_count: int32      # number of data points
...
class CameraPosition(dj.Imported):
        definition = """ # Raw per-chunk Position from Camera (v-0.4.7)
    -> Camera
    -> acquisition.Chunk
    ---
    sample_count: int32      # number of data points
...
class ActivityWeightScaleWeightSubject(dj.Imported):
        definition = """ # Raw per-chunk WeightSubject from ActivityWeightScale (v-0.4.7)
    -> ActivityWeightScale
    -> acquisition.Chunk
    ---
    sample_count: int32      # number of data points
...
class ActivityWeightScaleWeightRaw(dj.Imported):
        definition = """ # Raw per-chunk WeightRaw from ActivityWeightScale (v-0.4.7)
    -> ActivityWeightScale
    -> acquisition.Chunk
    ---
    sample_count: int32      # number of data points
...
class ActivityWeightScaleBaselineEvent(dj.Imported):
        definition = """ # Raw per-chunk BaselineEvent from ActivityWeightScale (v-0.4.7)
    -> ActivityWeightScale
    -> acquisition.Chunk
    ---
    sample_count: int32      # number of data points
...
class FeederRetriedDelivery(dj.Imported):
        definition = """ # Raw per-chunk RetriedDelivery from Feeder (v-0.4.7)
    -> Feeder
    -> acquisition.Chunk
    ---
    sample_count: int32      # number of data points
