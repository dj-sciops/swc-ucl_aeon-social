...
# Path to exported chunk metadata from production DB (aeon-db2).
# Generated by: python -m aeon.dj_pipeline.scripts.explore_prod_ephys_data --export
# Set via --chunk-export CLI arg or leave None to use ingest_chunks() as before.
CHUNK_EXPORT_PATH = None

# Skip spike alignment verification (requires Clock.bin + KS .bin files).
# Set via --skip-alignment CLI flag.
...
def step_06_import_chunks_from_export():
    """Import EphysChunk entries from exported production DB JSON.

    Alternative to step_06_ingest_chunks() — used when raw data files
    (AmplifierData.bin, Clock.bin) are not available (e.g., Works deployment).

    Requires: CHUNK_EXPORT_PATH set to the path of prod_ephys_chunk_export.json
    (generated by explore_prod_ephys_data.py --export).
    """
    print_header(6, "Import EphysChunks from production export")
    ...
    export_path = Path(CHUNK_EXPORT_PATH)
    if not export_path.exists():
        print_fail(f"Chunk export file not found: {export_path}")
        return False
    ...
    print_info(f"Loaded {len(chunks)} chunks from {export_path}")
    print_info(f"Source DB: {export_data.get('source_db', '?')}")

    # Convert ISO strings back to datetime objects
    for c in chunks:
        for key in ("chunk_start", "chunk_end", "epoch_start"):
            if key in c and isinstance(c[key], str):
                c[key] = datetime.fromisoformat(c[key])

    # Insert into EphysChunk (master table only — no File or SyncModel parts)
    # Verify no duplicates — don't silently skip
    pk_fields = ("experiment_name", "subject", "insertion_number", "chunk_start")
    existing_keys = (ephys.EphysChunk & {"experiment_name": EXPERIMENT_NAME}).fetch("KEY")
    existing_set = {
        tuple(sorted({k: row[k] for k in pk_fields}.items()))
        for row in existing_keys
    }
    new_chunks = [
        c for c in chunks
        if tuple(sorted({k: c[k] for k in pk_fields}.items()))
        not in existing_set
    ]
    if len(new_chunks) < len(chunks):
        n_dup = len(chunks) - len(new_chunks)
        print_info(f"  {n_dup} chunks already exist in DB — inserting {len(new_chunks)} new")
    if new_chunks:
        ephys.EphysChunk.insert(new_chunks)

    inserted = len(ephys.EphysChunk & {"experiment_name": EXPERIMENT_NAME})
    print_ok(f"EphysChunk entries: {inserted}")
    if inserted > 0:
        first = (ephys.EphysChunk & {"experiment_name": EXPERIMENT_NAME}).fetch(
            "chunk_start", order_by="chunk_start", limit=1
        )[0]
        last_end = (ephys.EphysChunk & {"experiment_name": EXPERIMENT_NAME}).fetch(
            "chunk_end", order_by="chunk_start DESC", limit=1
        )[0]
        print_info(f"  First chunk_start: {first}")
        print_info(f"  Last chunk_end:    {last_end}")

    return True


def step_06_ingest_chunks():
    """Ingest EphysChunks — from raw data or from exported production JSON."""
    if CHUNK_EXPORT_PATH is not None:
        return step_06_import_chunks_from_export()

    # Original behavior: discover from raw files
